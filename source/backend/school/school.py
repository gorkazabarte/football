...
def handler(event, context) -> dict | None:
    ...
    table_name = f'{ENV}-{APP_NAME}-school'

    try:
        if check_request_type(request_type, 'GET'):
            if check_request_path(request_path, 'schools'):
                print(f'[INFO] /GET /schools')
                return get_table(table_name=table_name)

            elif check_request_path(request_path, 'school'):
                school_name = get_path_params(request_path, 'school')
                print(f'[INFO] /GET /school/{school_name}')
                return get_item(item_name=school_name, table_name=table_name)

            elif check_request_type(request_type, 'POST'):
                if check_request_path(request_path, 'school'):
                    school_name = get_path_params(request_path, 'school')
                    print(f'[INFO] /POST /school/{school_name}')
                    raw_body = event.get('body', {})
                    body = loads(raw_body) if isinstance(raw_body, str) else raw_body
                    return post_item(item_name=school_name, table_name=table_name, **body)
