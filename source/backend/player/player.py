...
def handler(event, context) -> dict | None:
    ...
    table_name = f'{ENV}-{APP_NAME}-player'
    ...
    try:
        if check_request_type(request_type, 'GET'):
            if check_request_path(request_path, 'players'):
                ...
                player_name = get_path_params(request_path, 'player')
                print(f'[INFO] /GET /player/{player_name}')
                return get_item(item_name=player_name, table_name=table_name)

            elif check_request_type(request_type, 'POST'):
                if check_request_path(request_path, 'player'):
                    player_name = get_path_params(request_path, 'player')
                    print(f'[INFO] /POST /player/{player_name}')
                    raw_body = event.get('body', {})
                    body = loads(raw_body) if isinstance(raw_body, str) else raw_body
                    return post_item(item_name=player_name, table_name=table_name, **body)
